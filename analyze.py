...
def main(path):
    """Output some information about the WritingSample instance to stdout."""
    ...
    print("Example passive sentence from document: " +
          Document.passive_sentences[-1])
    ...
    print("Readability (Flesch Reading Ease: " +
          str(Document.readability_flesch_re))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--title', help="set the title of the document to\
                        be analyzed", action="store", type=str)
    parser.add_argument('-u', '--user', help="set the name of the current\
                        user", action="store", type=str)
    parser.add_argument('-v', '--verbose', help="detailed output",
                        action="store_true")
    parser.add_argument('-w', '--writer',
                        help="the author of the paper to be analyzed",
